...
find . -type d \( -name "__pycache__" -o -name "venv" -o -name ".pytest_cache" \) -prune -o \( -name "*.py" -o -name "*.md" -o -name "*.sh" \) -print > /tmp/project_files.txt
#echo './requirements.txt' >> /tmp/project_files.txt
#echo './openapi/openapi.yaml' >> /tmp/project_files.txt
#echo './task_plan.txt' >> /tmp/project_files.txt
#echo './instructions_for_gtp.txt' >> /tmp/project_files.txt
